...
| `get_weather_forecast` | 1〜7日の日別天気予報の取得 | 最高/最低気温、降水確率、天気状態、曜日付き日付を返します。 |

Tool 実行ログには、ツール名、引数、成功・失敗、完了時刻、所要時間が表示されます。

天気ツールでは、`東京` や `大阪` のような日本語地名と、`東京の天気予報` のように用途語が混ざった入力を地名候補へ正規化してから Open-Meteo の geocoding を行います。天気予報の日付は Tool 側で曜日付きに整形し、モデル側の曜日推測に依存しないようにしています。
