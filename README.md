...
エージェントモードでは、Gemma4 が `get_current_weather` を呼び出せる構成になっています。サーバー側で Open-Meteo API を呼び出し、その結果を tool message としてモデルに戻して最終回答を生成します。
...
- Tool Calling は現在、エージェントモードの機能検証用として天気取得ツールのみ実装しています。
